    ...
    def _build_matrices(self):
        ...
        if self.derivative_order == 1:  # First derivatives
            ...
            if self.convergence_order == 2:
                ...
                matrix[0, 0:5] = np.array([35 / 12, -26 / 3, 19 / 2, -14 / 3, 11 / 12])

                matrix[1, 0:5] = np.array([11 / 12, -5 / 3, 1 / 2, 1 / 3, -1 / 12])

                matrix[-2, -5:] = np.array([-1 / 12, 1 / 3, 1 / 2, -5 / 3, 11 / 12])

                matrix[-1, -5:] = np.array([11 / 12, -14 / 3, 19 / 2, -26 / 3, 35 / 12])
            elif self.convergence_order == 6:

                # Interior points - central difference coefficients

                for i in range(3, self.N - 3):
                    matrix[i, i - 3:i + 4] = np.array(
                        [2 / 180, -27 / 180, 270 / 180, -490 / 180, 270 / 180, -27 / 180, 2 / 180])

                # Specialized boundary stencils

                matrix[0, 0:7] = np.array([45 / 12, -154 / 12, 214 / 12, -156 / 12, 61 / 12, -10 / 12, 1 / 12])

                matrix[1, 0:7] = np.array([10 / 12, -15 / 12, -4 / 12, 14 / 12, -6 / 12, 1 / 12, 0])

                matrix[2, 0:7] = np.array([2 / 180, -27 / 180, 270 / 180, -490 / 180, 270 / 180, -27 / 180, 2 / 180])

                matrix[-3, -7:] = np.array([2 / 180, -27 / 180, 270 / 180, -490 / 180, 270 / 180, -27 / 180, 2 / 180])

                matrix[-2, -7:] = np.array([0, 1 / 12, -6 / 12, 14 / 12, -4 / 12, -15 / 12, 10 / 12])

                matrix[-1, -7:] = np.array([1 / 12, -10 / 12, 61 / 12, -156 / 12, 214 / 12, -154 / 12, 45 / 12])

        elif self.derivative_order == 3:  # Third derivatives
            if self.convergence_order == 2:
                # Try upwind-biased stencil for better stability
                # Interior points
                for i in range(2, self.N - 2):
                    matrix[i, i - 2:i + 3] = np.array([1 / 6, -1, 1 / 2, 1 / 3, -1 / 12])

                # Specialized boundary stencils (upwind-biased)
                matrix[0, 0:5] = np.array([-5 / 6, 2, -13 / 6, 2 / 3, -1 / 12])
                matrix[1, 0:5] = np.array([1 / 6, -1, 1 / 2, 1 / 3, -1 / 12])
                matrix[-2, -5:] = np.array([1 / 6, -1, 1 / 2, 1 / 3, -1 / 12])
                matrix[-1, -5:] = np.array([-1 / 12, 2 / 3, -13 / 6, 2, -5 / 6])
